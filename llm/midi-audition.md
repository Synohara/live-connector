...
2. 印刷した AudioTrack に対して `render` ツール（`apps/extension/src/tools/render.ts`）を適用し、`startTime` / `endTime`（beats）範囲で WAV を書き出す。長尺は `background:true` + `do` read `MATCH (j:RenderJob) RETURN j` で照会する。
...
MidiClip の `notes`（`do` read で取得）を汎用シンセ（soundfont / 簡易合成）で WAV 化し、タイミング・アレンジ・和声・ノート整合を検証する近似経路は技術的には可能だが、本リポジトリの射程外とする（記録）。理由:
...
- 実装する場合は `render` のジョブ方式（RenderJob 仮想ラベル）と同じ応答方式を共有できる。採用時は別 Issue に分割する。
...
関連: #21（AudioTrack のレンダリング・解析）、#55（render のジョブ方式）、#13 / #14 / #15（生成系）。
